...
interface ProductRepository : JpaRepository<ProductJpaEntity, Long> {
    ...
    @Query(value = """
        select p.id, p.number, p.brand, p.image_url, p.store, p.category, p.price, p.first_option, p.second_option, p.third_option, p.is_delete, p.created_date, p.last_modified_date, p.name
        from product p
        join basket b
